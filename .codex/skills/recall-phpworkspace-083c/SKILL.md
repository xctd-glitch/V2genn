...
1. **Session Start**: When user mentions a feature/module, search for related memories first
   - User says "let's work on authentication" → `"E:\.genv2\.codex\skills\recall-phpworkspace-083c/recall.sh" authentication`
...
3. **After Problem Solving**: When you've solved a tricky issue
   - Found a non-obvious bug → `"E:\.genv2\.codex\skills\recall-phpworkspace-083c/remember.sh" lesson "description"`
   - Made an important decision → `"E:\.genv2\.codex\skills\recall-phpworkspace-083c/remember.sh" architecture "description"`
...
5. **When memory looks stale or misleading**
   - Use `"E:\.genv2\.codex\skills\recall-phpworkspace-083c/forget.sh" "<keywords>"` to review candidate entries
...
| Action | Command |
|--------|---------|
| Search memories | `"E:\.genv2\.codex\skills\recall-phpworkspace-083c/recall.sh" <keywords>` |
| Save memory | `"E:\.genv2\.codex\skills\recall-phpworkspace-083c/remember.sh" <category> "<content>"` |
| Review stale/outdated candidates | `"E:\.genv2\.codex\skills\recall-phpworkspace-083c/forget.sh" "<keywords>"` |
...
```bash
"E:\.genv2\.codex\skills\recall-phpworkspace-083c/recall.sh" terminal session
"E:\.genv2\.codex\skills\recall-phpworkspace-083c/recall.sh" "error handling"
"E:\.genv2\.codex\skills\recall-phpworkspace-083c/remember.sh" lesson "node-pty requires explicit shell path on macOS"
"E:\.genv2\.codex\skills\recall-phpworkspace-083c/remember.sh" architecture "Use EventBus for cross-service communication"
"E:\.genv2\.codex\skills\recall-phpworkspace-083c/forget.sh" "old auth pattern"
```
